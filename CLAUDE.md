...
Routing now lives in **one** place: `projects/skeure-education/website-build/AGENTS.md` (note `CLAUDE.md` there is a symlink to it). The global half was removed on 2026-08-15 when `~/.claude/CLAUDE.md` was swapped for ECC's verbatim — see `decisions/log.md`. Sibling projects (`website-v2`, `website-v3`) do not inherit that file; symlink or copy it in if they need the rules. Editing plugin frontmatter is not safe — it gets clobbered on plugin update — so anything involving `impeccable` or ECC is enforced by instruction rather than by flag.
