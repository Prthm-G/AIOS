# website-v4 — THE LONG OPEN

One continuous shot: the laptop starts shut at the top of the homepage and opens
across the entire page, one family doubt at a time, until it stands fully open behind
the invitation to talk. It never closes.

- **Choreography lives in ONE file:** `src/data/filmBeats.ts`. Reordering beats there
  and in `src/app/page.tsx` together re-cuts the film; then re-run
  `bash scripts/build-frames.sh` (regenerates the graded desktop scrub AND the mobile
  beat stills from the same map).
- **Engine:** `src/components/film/FilmStage.tsx` (desktop canvas scrub, measured
  glass table, `?jump=<0..1>` + `window.__ready` dev contract).
- **Mobile is its own film:** 7 stills at the exact beat angles (~104KB). Phones never
  fetch the 61-frame desktop scrub.
- **The doubt spine is placeholder** — seeded from `src/content/faqs`. Swap in the
  real questions (in the order they arrive in counselling chats) when Pratham
  supplies them.
- Interior pages, metadata, and JSON-LD inherited verbatim from website-v3 so SEO
  does not regress. v3 remains the live site until this is reviewed and cut over.

Verify: `npm run build && npx next start -p 3400`, then screenshot any film position
with `?jump=<fraction>` once `window.__ready === true`.
